...
class Field(FieldBase):
    ...
    *curves* and *surfaces*, which correspond to shapely objects
    ...
    def _samples(spawn, obstacles, radius=0.3):
        ...
        print(vertices)
        print(seed)
        #print(polygon_sample(vertices, seed))
        #return [5, 5]
        return polygon_sample(vertices, seed)
    ...
    @lru_cache()
    def shortest_path_target(self, in_finlandia, step, index, radius):
        ...
        if in_finlandia == False:
            return shortest_path(self.meshgrid(step), self.domain, targets,
                             self.obstacles_finlandia, radius)
        else:
            return shortest_path(self.meshgrid(step), self.domain, targets,
                             self.obstacles, radius)

    @lru_cache()
    def direction_map_obstacles(self, in_finlandia, step):
        if in_finlandia == False:
            return direction_map_obstacles(self.meshgrid(step), self.obstacles_finlandia)
        else:
            return direction_map_obstacles(self.meshgrid(step), self.obstacles)

    @lru_cache()
    def navigation_to_target(self, in_finlandia, index, step, radius, strength):
        if not self.targets:
        ...
        #if index == 5:
        #    print('vitonen')
        dir_map_targets, dmap_targets = self.shortest_path_target(in_finlandia, step, index, radius)
        dir_map_obs, dmap_obs = self.direction_map_obstacles(in_finlandia, step)
        dir_map = obstacle_handling(dmap_obs, dir_map_obs, dir_map_targets,
                                    radius, strength)

        #if index == 5:
        #    #print('vitonen onnistui')
        #    np.savetxt('new_dmap.out', dmap_targets, delimiter=',')
        #    np.savetxt('new_dmapX.out', dmap_targets)
        #    np.savetxt('new_gradienttiX.out', dir_map_targets[0], delimiter=',')
        #    np.savetxt('new_gradienttiY.out', dir_map_targets[1], delimiter=',')
        #    mesh = self.meshgrid(step)
        #    np.savetxt('new_gridX.out', mesh[0][0], delimiter=',')
        #    np.savetxt('new_gridY.out', mesh[0][1], delimiter=',')
        #    print('heppista')
