...
from crowddynamics.core.sampling import polygon_sample, random_sample_triangle, triangle_area_cumsum
...
class TranslationalMotion(HasTraits):
    ...
    in_helsinki = Array(
        default_value = False,
        dtype=np.bool,
        help='In Helsinki hall',
        symbol='in_helsinki'
    )
    in_orchestra = Array(
        default_value = False,
        dtype=np.bool,
        help='In orchestra foyer',
        symbol='in_orchestra'
    )
    in_finlandia = Array(
        default_value = False,
        dtype=np.bool,
        help='In Finlandia hall',
        symbol='in_finlandia'
    )
    in_finlandia_extended = Array(
        default_value = False,
        dtype=np.bool,
        help='In extended Finlandia hall',
        symbol='in_finlandia_extended'
    )
    position = Array(
        default_value=(0, 0),
        dtype=np.float64,
...
class Agents(AgentsBase):
    ...
    def add_non_overlapping_group(self, groupname, group, position_gen, position_iter, spawn, obstacles=None):
        """Add group of agents
        ...
        group_data = np.load("agents_initialization_conference.npy")
        ...
        while index < group.size:
            ...
            if position_gen:
                ...
                if groupname == "group_follower_finlandiahall":
                    new_agent.radius = group_data[index][7]
                    new_agent.mass = group_data[index][11]
                    new_agent.target_velocity = group_data[index][13]
                    new_agent.position = group_data[index][21]

                if groupname == "group_follower_piazza_3":
                    new_agent.radius = group_data[index+300][7]
                    new_agent.mass = group_data[index+300][11]
                    new_agent.target_velocity = group_data[index+300][13]
                    new_agent.position = group_data[index+300][21]

                if groupname == "group_follower_piazza_1":
                    new_agent.radius = group_data[index+400][7]
                    new_agent.mass = group_data[index+400][11]
                    new_agent.target_velocity = group_data[index+400][13]
                    new_agent.position = group_data[index+400][21]

                if groupname == "group_follower_restaurant":
                    new_agent.radius = group_data[index+600][7]
                    new_agent.mass = group_data[index+600][11]
                    new_agent.target_velocity = group_data[index+600][13]
                    new_agent.position = group_data[index+600][21]

                if groupname == "group_follower_foyer":
                    new_agent.radius = group_data[index+700][7]
                    new_agent.mass = group_data[index+700][11]
                    new_agent.target_velocity = group_data[index+700][13]
                    new_agent.position = group_data[index+700][21]

                if groupname == "group_follower_helsinkihall":
                    new_agent.radius = group_data[index+800][7]
                    new_agent.mass = group_data[index+800][11]
                    new_agent.target_velocity = group_data[index+800][13]
                    new_agent.position = group_data[index+800][21]

                if groupname == "group_follower_orchestra":
                    new_agent.radius = group_data[index+1000][7]
                    new_agent.mass = group_data[index+1000][11]
                    new_agent.target_velocity = group_data[index+1000][13]
                    new_agent.position = group_data[index+1000][21]    
