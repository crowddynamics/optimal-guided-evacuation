...
class Agents(AgentsBase):
    ...
    def add_non_overlapping_group(self, groupname, group, position_gen, position_iter, spawn, obstacles=None):
        ...
        group_data = np.load("agents_initialization_simple.npy")
        #print(groupname)
        #print(group.size)
        ...
        while index < group.size:
            ...
            if position_gen:
                ...
                if groupname == "group_follower_spawn1":
                    new_agent.radius = group_data[index][7]
                    new_agent.mass = group_data[index][11]
                    new_agent.target_velocity = group_data[index][13]
                    new_agent.position = group_data[index][15] #in finlandia its index 23

                if groupname == "group_follower_spawn2":
                    new_agent.radius = group_data[index+25][7]
                    new_agent.mass = group_data[index+25][11]
                    new_agent.target_velocity = group_data[index+25][13]
                    new_agent.position = group_data[index+25][15]

                if groupname == "group_follower_spawn3":
                    new_agent.radius = group_data[index+50][7]
                    new_agent.mass = group_data[index+50][11]
                    new_agent.target_velocity = group_data[index+50][13]
                    new_agent.position = group_data[index+50][15]

                if groupname == "group_follower_spawn4":
                    new_agent.radius = group_data[index+75][7]
                    new_agent.mass = group_data[index+75][11]
                    new_agent.target_velocity = group_data[index+75][13]
                    new_agent.position = group_data[index+75][15]

                if groupname == "group_follower_spawn5":
                    new_agent.radius = group_data[index+100][7]
                    new_agent.mass = group_data[index+100][11]
                    new_agent.target_velocity = group_data[index+100][13]
                    new_agent.position = group_data[index+100][15]

                if groupname == "group_follower_spawn6":
                    new_agent.radius = group_data[index+125][7]
                    new_agent.mass = group_data[index+125][11]
                    new_agent.target_velocity = group_data[index+125][13]
                    new_agent.position = group_data[index+125][15]

            #print(new_agent.radius)
            #print(new_agent.mass)
            #print(new_agent.target_velocity)
            #print(new_agent.position)
