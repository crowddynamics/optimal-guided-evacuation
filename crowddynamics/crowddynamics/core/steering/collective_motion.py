    ...
    for i in indices[is_follower]:
        ...
        for k in np.argsort(distances):
            ...
            #        behind_obstacle += 1
                ...
                #is_heading_away, _ = herding_relationship(position[i], position[j], velocity[i], velocity[j], phi)

                #if is_heading_away and index_leader[i] == NO_LEADER:
                if not is_obstacle_between_points(position[i], position[j], obstacles) and index_leader[i] == NO_LEADER:
                    #heading_away += 1
                    # We see this leader. Remember it and follow.
                    #if index_leader[i] == NO_LEADER:
                    index_leader[i] = j
                    leader = index_leader[i]
                    #print("collective_2")
                    #print(target[leader])
...
def leader_follower_interaction(
    ...
    target, has_strategy = leader_follower_interaction_brute(
    ...
    has_guide = has_strategy & agents['is_follower']
    #print(is_lost) # tämä on OK
    # print("active familiar exit")
    # print(agents['familiar_exit'][np.nonzero(agents['active'])])
    # print("guidillisten targetit")
    # print(target[has_guide])
    # print("guidittomien familiarit")
    # print(agents['familiar_exit'][is_lost])
    agents['target'][has_guide] = target[has_guide]
