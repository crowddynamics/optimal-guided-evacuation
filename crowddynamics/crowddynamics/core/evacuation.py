...
from numba import i8, f8, optional, boolean
...
@numba.jit((f8[:, :], f8[:, :], boolean[:], typeof(obstacle_type_linear)[:], f8),
           nopython=True, nogil=True, cache=True)
def exit_detection(center_door, position, in_finlandia, obstacles, detection_range):
    ...
    for i in range(n):
        # Ad hoc solution to set all agents close to Exit 0, to go there
        if position[i][0] >= 120 and position[i][1] >= 27:
            detected_exit[i] = 0
            has_detected[i] = True
            continue
        else:
